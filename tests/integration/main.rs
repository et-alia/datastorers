...
use datastorers::{
    ...
    IdentifierNone, Kind, Operator, QueryProperty, DatastorersQuery, DatastoreValue
};
...
// GENERATED BY MACRO CODE (or not yet, but it shall be)
pub enum TestEntityFilter {
    PropString(Operator, String),
    PropInt(Operator, i64),
}

impl QueryProperty for TestEntityFilter {

    fn get_property_name(&self) -> &'static str {
        match self {
            TestEntityFilterProp::PropString(_, _) => "Name",
            TestEntityFilterProp::PropInt(_, _) => "int_property"
        }
    }

    fn get_operator(&self) -> Operator {
        match self {
            TestEntityFilterProp::PropString(o, _) => *o,
            TestEntityFilterProp::PropInt(o, _) => *o,
        }
    }
    ...
    fn get_value(self) -> Value {
        // TODO - use serialize?
        match self {
            TestEntityFilterProp::PropString(_, v) => {
                let mut val = DatastoreValue::empty();
                val.string_value = Some(v);
                val.into()
            },
            TestEntityFilterProp::PropInt(_, v) => {
                let mut val = DatastoreValue::empty();
                val.integer_value = Some(v);
                val.into()
            },
        }
    }
...
impl TestEntity {
    ...
    fn get_query_builder() -> DatastorersQuery<TestEntity, TestEntityFilterProp> {
...
#[tokio::test]
async fn test_get_by_query() -> Result<(), DatastorersError> {
    ...
    let fetch_result = TestEntity::get_query_builder()
        .filter_by(TestEntityFilter::PropString(Operator::Eq, expected_result_entity.prop_string))
        .query_one(&connection).await;

    // Success
    let fetched_entity = match fetch_result
        {
            Ok(e) => e,
